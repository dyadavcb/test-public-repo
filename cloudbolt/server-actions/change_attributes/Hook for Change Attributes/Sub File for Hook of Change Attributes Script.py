...
def generate_options_for_owner(server=None, servers=None, profile=None, **kwargs):
    """Generates the options for the owner field in the change attributes dialog.

    Returns:
        dict: A dictionary containing the options for the owner field. For example:
        {
            'options': [(None, '(no change)'), (1, 'admin'), (2, 'user1')],
            'initial_value': (None, '(no change)'),
            'required': False
        }
    ...
    # Get the user's preferred scheme for displaying a username in forms.
    # Available options include: full_name, username, first_initial_last, etc.
    user_display_option = GlobalPreferences.get().default_user_display_scheme
    ...
    for profile in userprofile_qs:
        user_display = getattr(profile, user_display_option, profile.username)
        options.append((profile.id, user_display))

    if server and server.owner:
        initial = (
            server.owner.id,
            getattr(server.owner, user_display_option, server.owner.username),
        )
        if initial not in options:
            options.append(initial)
        ret["initial_value"] = initial

    ret["options"] = sorted(options, key=lambda x: x[1].lower())
...
def generate_options_for_group(server=None, servers=None, profile=None, **kwargs):
    ...
    ret = {
        "options": [
            (g.id, g.standalone_name) for g in Group.objects_for_profile(profile)
        ]
    }
    if server:
        initial = (server.group.id, server.group.standalone_name)
...
def generate_options_for_management_groups(
    ...
    if server and server.management_groups.exists():
        ...
        for group_id in [
            ...
            ret["options"].append((group.id, group.standalone_name))
        ret["initial_value"] = initial
    if not server:
...
def run(*args, request=None, server=None, servers=None, profile=None, **kwargs):
    ...
    if kwargs.get("bulk_changes", ""):
        """
        In the 'Bulk Change Resources' action, we perform initial validation to identify any errors.
        If there are no validation errors, the servers are updated. Otherwise, an error list is returned.
        """
        failed_validation_error_result = []
        passed_validation_servers_result = []
        for server in servers:
            kwargs["environment"] = server.environment.id
            validation_errors = gather_validation_errors(kwargs)
            if validation_errors:
                failed_validation_error_result.append(" ".join(validation_errors))
            else:
                passed_validation_servers_result.append(str(server.id))
        if failed_validation_error_result:
            return (
                "group_validation",
                failed_validation_error_result,
                passed_validation_servers_result,
