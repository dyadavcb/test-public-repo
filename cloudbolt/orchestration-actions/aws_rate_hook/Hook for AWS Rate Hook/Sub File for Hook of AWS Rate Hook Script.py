...
def _compute_aws_rate(region, os_build, instance_type, environment):
    ...
    if not resource_handler:
        # This only happens in edge cases.  Like an environment was edited and the RH was removed.
        # We should not guess and risk looking at someone else's billing data.  Return None
        return None
    resource_handler = resource_handler.cast()
